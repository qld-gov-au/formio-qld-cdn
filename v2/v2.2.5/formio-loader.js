...
module.exports = {
  ...
  i18n: {
    en: {
      ...
      error: '<h2 class="alert-heading">Please check your answers</h2>'
    }
  }
};
...
const requestPluginHandler = (requestArgs, opts) => {
  ...
  if (requestArgs !== null && requestArgs !== void 0 && requestArgs.formio) {
    ...
    if (requestArgs.formio.options) {
      ...
      if (requestArgs.formio.options.project) Formio.setProjectUrl(requestArgs.formio.options.project);
      if (requestArgs.formio.options.base) Formio.setBaseUrl(requestArgs.formio.options.base);
    }
  }
...
const initFormioInstance = (elem, opts) => {
  ...
  Formio.createForm(elem, formUrl, combinedOptions).then(form => {
    ...
    // default controller
    createForm_controller(callbackProps);
